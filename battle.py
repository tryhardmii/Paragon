...
def battle(player,area,difficulty):
    ...
    area -= 1
    difficulty -=1
    spawnchances = [] #random monster based on spawnrate
    for num in range(len(monsterlist[area])):
        spawnchances.append([x+num for x in range(monsterlist[area][num].spawnrate)])
    ...
    while game_active==True:
        ...
        if stage<6:
            ...
            if stage==7:
                ...
                resulttext = printfont.render("you flippin suck",True,(255,255,255))
                player.tilex = 101
                player.tiley = 181
                player.HP = 100
                sigma = False
    ...
    if game_active == False:
        if sigma == True:
            print(player.mobsdefeated)
            print(selectedmonster.name)
            print(player.mobsdefeated["wizard"], player.mobsdefeated[selectedmonster.name])
