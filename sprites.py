    ...
    def update(self, player, speed, velocity):
        ...
        if self.following == False:
            ...
            try:
                ...
                print(((player.posx - self.rect.centerx) / 32, (player.posy - self.rect.centery) / 32))
                pass
class chest(pygame.sprite.Sprite):
    def __init__(self, name, tilex, tiley, item):
        pygame.sprite.Sprite.__init__(self)
        self.type = "chest"
        self.name = name
        self.tilex = tilex
...
class obstacle(pygame.sprite.Sprite):
    ...
    def update(self, player):
        pass
        # code to display surfaces for bug testing
...
class tallgrass(obstacle):#grass
    ...
    def encounter(self, player,difficulty):
...
class background(pygame.sprite.Sprite):
    def __init__(self, image1,image2, posx, posy):
        ...
        self.image2 = pygame.image.load(image2).convert_alpha() #2 layers for overlap
        self.posx = posx
        self.posy = posy
...
class question(pygame.sprite.Sprite):
    def __init__(self, difficulty, posx, posy):
        ...
        # else: for open text box i will kill myself
        # self.answerbox =
    ...
    def __init__(self,list1,posx,posy):
        ...
        for x in list1:
            for z in list1[x]:
                for y in range(len(inventoryitems[x])):
                    if z==inventoryitems[x][y]:
                        inventorycounts[x][y]+=1
        self.weapontitle = printfont.render('WEAPONS:',False,(255,255,255))
      ...
      #  for x in range(len(inventoryitems["drops"])):
       ...
       #     self.dropsitems.append(inventoryItem(inventoryitems["drops"][x],x*164+192,600,inventorycounts["drops"][x]))
        self.dropsitems = [inventoryItem(inventoryitems["drops"][x],x*164+192,600,inventorycounts["drops"][x]) if x<=6 else inventoryItem(inventoryitems["drops"][x],(x-7)*164+192,750,inventorycounts["drops"][x]) for x in range(len(inventoryitems["drops"]))]
    ...
    def checkpressed(self,event):
        ...
        if self.buybutton.checkpressed(event) == True:
            ...
            if self.selecteditem in self.player.inventory["weapon"] + self.player.inventory["defence"]:
                self.errortext = self.printfont.render("you already own this item",False,(255,0,0))
            else:
        ...
        if self.sellbutton.checkpressed(event) == True:
            ...
            if self.selecteditem!=False:
                if self.selecteditem.itemtype == "weapon" and len(self.player.inventory["weapon"])==1:
                    self.errortext = self.printfont.render("you need one weapon", False, (255, 0, 0))
                else:
...
class questinfo(pygame.sprite.Sprite):
    def __init__(self,player,position,npc,file):
        from quests import questupdate
        self.questlist = questupdate(player,npc,file,"questlist")
        pygame.sprite.Sprite.__init__(self)
        self.file = file
